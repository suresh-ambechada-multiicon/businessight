        ...
        try:
            ...
            with db._engine.connect() as conn:
                ...
                for col_name, data_type, is_nullable in columns:
                    ...
                    empty_count = 0
                    if data_type.lower() in ['character varying', 'varchar', 'text', 'char']:
                        empty_result = conn.execute(text(f"""
                ...
                if duplicate_rows > 0:
                    ...
                    quality_issues.append(f"    - {duplicate_rows} duplicate rows found")
                
                if quality_issues:
                    lines.extend(["", "Summary Issues:"] + quality_issues)
                else:
                    lines.extend(["", "OK: Data quality looks good - no significant issues found"])
                
                elapsed = round((time.time() - start) * 1000, 2)
                logger.info("Tool: analyze_data_quality", extra={
                    **_ctx, "table": table_name, "rows": total_rows, "issues": len(quality_issues), "time_ms": elapsed
                })
                
                return "\n".join(lines)

        except Exception as e:
            logger.error("analyze_data_quality failed", extra={
