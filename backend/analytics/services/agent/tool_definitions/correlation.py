        ...
        try:
            ...
            with db._engine.connect() as conn:
                ...
                if len(numeric_cols) < 2:
                    return f"Need at least 2 numeric columns. Found: {len(numeric_cols)}"
                ...
                for i, col1 in enumerate(numeric_cols):
                    for j, col2 in enumerate(numeric_cols):
                        ...
                        # Get paired values
                        pairs = [(row[i], row[j]) for row in sample_data 
                ...
                # Strong correlations
                strong = [(c1, c2, corr, n) for c1, c2, corr, n in correlations if abs(corr) > 0.5]
                moderate = [(c1, c2, corr, n) for c1, c2, corr, n in correlations 
                           if 0.3 < abs(corr) <= 0.5]
                ...
                if strong:
                    ...
                    for c1, c2, corr, n in strong[:10]:
                        ...
                        lines.append(f"  {c1} <-> {c2}: {corr:+.3f} ({strength}, {n} pairs)")
                ...
                logger.info("Tool: find_correlations", extra={
                    **_ctx, "table": table_name, "pairs": len(correlations), "time_ms": elapsed
                })
                
                return "\n".join(lines)

        except Exception as e:
            logger.error("find_correlations failed", extra={**_ctx, "table": table_name, "error": str(e)})
