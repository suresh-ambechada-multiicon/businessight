    ...
    def detect_outliers(table_name: str, column_name: str, method: str = "iqr") -> str:
        ...
        try:
            ...
            with db._engine.connect() as conn:
                ...
                # Interpretation
                if outlier_pct > 10:
                    lines.extend(["", "WARNING: High outlier percentage may indicate data quality issues."])
                elif outlier_pct > 5:
                    lines.extend(["", "Note: Moderate outlier percentage, worth investigating."])
                else:
                    lines.extend(["", "OK: Low outlier percentage indicates clean data."])
                
                elapsed = round((time.time() - start) * 1000, 2)
                logger.info("Tool: detect_outliers", extra={
                    **_ctx, "table": table_name, "column": column_name, "outliers": outlier_count, "time_ms": elapsed
                })
                
                return "\n".join(lines)

        except Exception as e:
            logger.error("detect_outliers failed", extra={
