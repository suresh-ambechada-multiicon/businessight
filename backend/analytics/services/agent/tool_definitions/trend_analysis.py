...
def create_trend_analysis_tool(db, ctx, _status, _ctx, _full_table, _quote_ident, _select_top):
    """Factory to create the analyze_trends tool."""

    @tool
    def analyze_trends(table_name: str, date_column: str, value_column: str, 
                        period: str = "day") -> str:
        """
        ...
        try:
            ...
            date_trunc = period_map.get(period, period_map["day"]).format(date_col=quoted_date)
            ...
            with db._engine.connect() as conn:
                ...
                period_growth = ((current_total - previous_total) / previous_total * 100) if previous_total else 0
                overall_growth = ((current_total - first_total) / first_total * 100) if first_total else 0
                ...
                last_3_avg = sum(totals[-3:]) / min(3, len(totals)) if len(totals) >= 3 else sum(totals) / len(totals)
                ...
                for period_date, total, avg, count in trend_data[-5:]:
                    lines.append(f"  {period_date}: {total:,.2f} (avg: {avg:,.2f}, count: {count})")
                
                elapsed = round((time.time() - start) * 1000, 2)
                logger.info("Tool: analyze_trends", extra={
                    **_ctx, "table": table_name, "trend": trend_direction, "time_ms": elapsed
                })
                
                return "\n".join(lines)

        except Exception as e:
            logger.error("analyze_trends failed", extra={
